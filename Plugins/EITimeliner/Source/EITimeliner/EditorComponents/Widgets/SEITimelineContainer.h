...
class EITIMELINER_API  SEITimelineContainer:public SCompoundWidget
{
	
public:
	SLATE_BEGIN_ARGS(SEITimelineContainer){}
		/** The current total range of frame indices */
		SLATE_ATTRIBUTE( FInt32Interval, ViewIndices )
		SLATE_ATTRIBUTE( FEIPreviewScene*, PreviewScene )
...
public:
	...
	TWeakPtr<FEITimelinerContext> GetPreviewInstanceExecutor() const;
	TWeakPtr<FEITimelinerContext> GetPreviewInstance() const;
private:
...
public:
	...
	FEIPreviewScene* mPreviewScene=nullptr;

	TWeakPtr<FEITimelinerContext> Context;
	
	TSharedPtr<SEITrackArea> TrackArea;
	
	TSharedPtr<SEITrackOutliner> Outliner;

	/** The time slider controller */
	TSharedPtr<FEITimeSliderController> TimeSliderController;
