#include "SKuruTimelineSplitterOverlay.h"

struct FCaptureLostEvent;
struct FPointerEvent;

void SKuruTimelineSplitterOverlay::Construct( const FArguments& InArgs )
{
	...
	{
		AddSlot()
		.Padding(TAttribute<FMargin>::Create(TAttribute<FMargin>::FGetter::CreateSP(this, &SKuruTimelineSplitterOverlay::GetSplitterHandlePadding, Index)))
		[
			SNew(SBox)
...
void SKuruTimelineSplitterOverlay::OnArrangeChildren( const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren ) const
{
...
FMargin SKuruTimelineSplitterOverlay::GetSplitterHandlePadding(int32 Index) const
{
...
FCursorReply SKuruTimelineSplitterOverlay::OnCursorQuery( const FGeometry& MyGeometry, const FPointerEvent& CursorEvent ) const
{
	return Splitter->OnCursorQuery(MyGeometry, CursorEvent);
}

FReply SKuruTimelineSplitterOverlay::OnMouseButtonDown( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
void SKuruTimelineSplitterOverlay::OnMouseCaptureLost(const FCaptureLostEvent& CaptureLostEvent)
{
	SetVisibility(EVisibility::SelfHitTestInvisible);
	SOverlay::OnMouseCaptureLost(CaptureLostEvent);
}

FReply SKuruTimelineSplitterOverlay::OnMouseButtonUp( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
FReply SKuruTimelineSplitterOverlay::OnMouseMove( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	return Splitter->OnMouseMove(MyGeometry, MouseEvent);
}

void SKuruTimelineSplitterOverlay::OnMouseLeave( const FPointerEvent& MouseEvent )
{
