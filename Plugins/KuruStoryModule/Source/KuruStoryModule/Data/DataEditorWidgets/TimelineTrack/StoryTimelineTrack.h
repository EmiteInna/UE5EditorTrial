 ...
 */
struct FStoryTrackPadding
{
	FStoryTrackPadding(float InUniform) 
		: Top(InUniform)
		, Bottom(InUniform) {}

	FStoryTrackPadding(float InTop, float InBottom) 
		: Top(InTop)
		, Bottom(InBottom) {}
...
#define STORYTIMELINE_DECLARE_BASE_TRACK(BaseClassName) \
public: \
...
#define STORYTIMELINE_DECLARE_TRACK(ClassName, BaseClassName) \
public: \
...
private: \
static const FName TypeName;
#define STORYTIMELINE_IMPLEMENT_TRACK(ClassName) \
const FName ClassName::TypeName = TEXT(#ClassName);




class FStoryTimelineTrack : public TSharedFromThis<FStoryTimelineTrack>
{
STORYTIMELINE_DECLARE_BASE_TRACK(FStoryTimelineTrack);

public:
	static const float OutlinerRightPadding;

	FStoryTimelineTrack(const FText& InDisplayName, const FText& InToolTipText, UKuruStoryClipData* InModel, bool bInIsHeaderTrack = false)
		: Model(InModel)
	...
	virtual ~FStoryTimelineTrack() {}

	/** Get the children of this object */
	const TArray<TSharedRef<FStoryTimelineTrack>>& GetChildren() { return Children; }

	/** Add a child track */
	void AddChild(const TSharedRef<FStoryTimelineTrack>& InChild);
	...
	UKuruStoryClipData* GetModel() const { return Model; }
	 ...
	 */
	bool Traverse_ChildFirst(const TFunctionRef<bool(FStoryTimelineTrack&)>& InPredicate, bool bIncludeThisTrack = true);
	 ...
	 * @return  true where the client prematurely exited the iteration, false otherwise
	 */
	bool Traverse_ParentFirst(const TFunctionRef<bool(FStoryTimelineTrack&)>& InPredicate, bool bIncludeThisTrack = true);
	 ...
	 * @return  true where the client prematurely exited the iteration, false otherwise
	 */
	bool TraverseVisible_ChildFirst(const TFunctionRef<bool(FStoryTimelineTrack&)>& InPredicate, bool bIncludeThisTrack = true);
	 ...
	 * @return  true where the client prematurely exited the iteration, false otherwise
	 */
	bool TraverseVisible_ParentFirst(const TFunctionRef<bool(FStoryTimelineTrack&)>& InPredicate, bool bIncludeThisTrack = true);

	/** Generate a widget for the outliner for this track */
	virtual TSharedRef<SWidget> GenerateContainerWidgetForOutliner(const TSharedRef<SStoryTrackOutlinerItem>& InRow);
	...
	const FStoryTrackPadding& GetPadding() const { return Padding; }
...
protected:
	...
	TSharedRef<SWidget> GenerateStandardOutlinerWidget(const TSharedRef<SStoryTrackOutlinerItem>& InRow, bool bWithLabelText, TSharedPtr<SBorder>& OutOuterBorder, TSharedPtr<SHorizontalBox>& OutInnerHorizontalBox);
...
protected:
	/** Child objects */
	TArray<TSharedRef<FStoryTimelineTrack>> Children;
